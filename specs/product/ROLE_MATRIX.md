...
### Current deployment: one account, two jobs

No `operations_manager` account is issued today — the `admin` account performs **both** the
Administrator and the Operations Manager duties. Consequences for the UI:

-   `admin` is treated as a **superset** of `operations_manager`: any capability marked `F`/`A`
    for Operations is also available to Admin, even where the matrix below shows `R`.
-   The admin console splits the two jobs with a **mode switch** rather than two roles — see
    [`../ux/NAVIGATION.md`](../ux/NAVIGATION.md) § Console mode switch. The mode is a view
    preference and grants nothing; the server still authorizes every call (BR-AUTH-4).
-   When an `operations_manager` account is issued, the matrix below applies as written and that
    role locks to the `operations` mode.
...
| M9 Logistics      | Route options / VRP / review / assign    |     —      |     F      |   F   |
